...
if st.session_state.app_locked:
    st.markdown("<div class='scaffold-lock'>", unsafe_with_html=True)
...
with col2:
    ...
    if st.session_state.current_analysis:
        ...
        with m_col1:
            ...
            st.markdown(f"<div class='metric-card'><b>Verdict</b><br><span style='font-size:20px;'>{status_text}</span></div>", unsafe_with_html=True)
        with m_col2:
            st.markdown(f"<div class='metric-card'><b>Marks Awarded</b><br><span style='font-size:20px;'>{res.get('overall_score_awarded')}</span></div>", unsafe_with_html=True)
        with m_col3:
            st.markdown(f"<div class='metric-card'><b>Deficiency Tier</b><br><span style='font-size:16px; color:#FF4B4B;'>{res.get('failure_tier')}</span></div>", unsafe_with_html=True)
        ...
        for i, line in enumerate(res.get("line_by_line_audit", [])):
            if err_idx is not None and i == err_idx:
                st.markdown(f"<div class='audit-line-error'>{line}</div>", unsafe_with_html=True)
            else:
                st.markdown(f"<div class='audit-line-correct'>{line}</div>", unsafe_with_html=True)
